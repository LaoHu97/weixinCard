...
<div>
  <div class="top">
    <div class="top-center">
      <div class="top-center-consumption">
        <p><i class="fa fa-bullhorn fa-lg"></i> 详情请咨询服务员</p>
      </div>
    </div>
  </div>
  <div class="center">
    <group>
      <selector title="款台：" placeholder="请选择款台" direction="right" :options="list1" v-model="numberOli" @on-change="onChange"></selector>
      <x-input title="金额：" placeholder="输入金额（10-9999）" :max="6" type="number" v-model="money" :show-clear="false" placeholder-align="left" text-align="left" :disabled='disabledMoney' v-bind:style="{ opacity: activeOpacity }"></x-input>
    </group>
  </div>
  ...
  <x-dialog v-model="showHideOnBlur" class="dialog-demo">
    ...
    <div class="onepx"></div>
    <p style="font-weight:bold">￥{{money}}&nbsp;<span style="color: rgb(204, 204, 204);text-decoration: line-through;font-size:12px;">{{money}}</span></p>
    <group>
...
export default {
  ...
  methods: {
    ...
    getempList() {
      var that = this;
      let sid = that.GetQueryString("sid")
      console.log(sid);
      
      that.$http({
        method: 'get',
        url: COURSES + `member/store/${sid}`,
        data: {
      ...
      if (that.type == "1" && reg.test(this.money)) {
        if (that.formRadioWx == "1") {
          that.$http({
            ...
            data: {
              ...
              'paidAmount': (Math.round(that.money * 100) - Math.round(that.vals * 100)).toString()
            }
          }).then(function(res) {
        ...
        that.$http({
          ...
          data: {
              'discount': this.GetQueryString("discount"),
              'memId': this.GetQueryString("memId"),
              'memberOpId': this.GetQueryString("mpopenId"),
              'payMode': 'ALI',
              'totalFee': Math.round(that.money * 100).toString(),
              'paidAmount': (Math.round(that.money * 100) - Math.round(that.vals * 100)).toString()
          }
          ...
          if (res.data.subCode === '000000') {
            that.$vux.loading.hide()
            window.location.href = res.data.data.aliPreOrderCO.codeUrl;
          } else {
            that.$vux.loading.hide();
            that.$vux.toast.show({
              text: res.data.subMsg,
    ...
    submitFormVIP() {
      ...
      console.log(that.pwd, that.password);
